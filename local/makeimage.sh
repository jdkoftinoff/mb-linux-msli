...
if [ -f "${DMITRI_IO_MAINT_DOWNLOAD_BIT}" ]
    then 
    ../../mbbl/mbbl-mkbootimage/pad-file -b 256 -s 32 -x "${DMITRI_IO_MAINT_DOWNLOAD_BIT}"
...
if [ -f "${DMITRI_IO_GNET_DOWNLOAD_BIT}" -a -f "${MBBL_ELF}" ]
    then
    ../../mbbl/mbbl-mkbootimage/pad-file -b 256 -s 32 -x "${DMITRI_IO_GNET_DOWNLOAD_BIT}"
...
if [ -f "${DMITRI_IO_AVB_DOWNLOAD_BIT}" ]
    then
    ../../mbbl/mbbl-mkbootimage/pad-file -b 256 -s 32 -x "${DMITRI_IO_AVB_DOWNLOAD_BIT}"
    echo "AVB tar file..."
    if [ -d update ]
	then
	rm -rf update
