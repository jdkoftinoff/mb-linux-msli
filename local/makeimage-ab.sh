...
&& echo -ne "\x1b[3D,   \ntar file..." \
&& (mkdir update 2>/dev/null ; true ) && rm -f update/mbbl.elf && cp "${HOME}"/dmitri_io_0/implementation/download.bit linux.bin.gz logo-1.bin.gz 8x12-font.bin.gz 16x24-font.bin.gz romfs.bin.gz dt.dtb identity.txt update && tar czf firmware.tar.gz update \
