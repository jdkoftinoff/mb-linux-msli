...
#ifndef NANO_TINY
static pid_t pid = -1;
	/* The PID of the vforked process in execute_command(), for use
...
#ifndef NANO_TINY
/* Send a SIGKILL (unconditional kill) to the vforked process in
 * execute_command(). */
...
bool execute_command(const char *command)
{
    ...
    if (pid == -1) {
	close(fd[0]);
	statusbar(_("Could not vfork"));
	return FALSE;
    }

    /* Before we start reading the vforked command's output, we set
...
const char *do_int_speller(const char *tempfile_name)
{
    ...
    /* The child process was not vforked successfully. */
    if (pid_spell < 0 || pid_sort < 0 || pid_uniq < 0) {
	close(uniq_fd[0]);
	return _("Could not vfork");
    }
