...
static int getter_section(void *cookie)
{
	...
	/* Ensure we don't wait at the getchar() forever if 
	 * there are any network problems
	 */
	if (alarm_counter == 0) {
		alarm(30);
	}
	...
	/* Reset the alarm every OS_BUF_SIZE characters */
	alarm_counter++;
	if (alarm_counter > OS_BUF_SIZE) {
		alarm(0);
		alarm_counter = 0;
	}
...
int cgi_extract_sections(output_buffer_function *writer)
{
	...
	/* Turn off any alarm that may have been left on in getter_section() */
	alarm(0);
