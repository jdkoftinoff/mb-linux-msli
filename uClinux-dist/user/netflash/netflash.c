...
int doversion;			/* check version information */
int dohardwareversion;		/* check hardware version information */
...
static int kill_processes_run = 0;	/* Has kill_processes() been run? */
...
int check_version_info(int offset, int removeversion, int failifnoversion);
...
void restartinit(void)
{
	...
	if (!docgi) {
		...
		killprocname("init", SIGCONT);  /* should kill both inits */
#else
		kill(1, SIGCONT);
#endif
	}
...
/* Call this function before decompressing, 
 * to strip the v2 crypto stuff from the image
 */
#ifdef CONFIG_USER_NETFLASH_CRYPTO_V2 
void strip_crypto_sig() {
 ...
 *	the image.
 */
int check_crypto_signature(void)
{
	struct header hdr;
	int hash_length;
...
#ifdef CONFIG_USER_NETFLASH_WITH_FILE
	killprocname("init", SIGTSTP);	/* should kill both inits */
#else
...
#endif
		printf("\r%5dK %3ld%%", (sgpos + sgsize) / 1024, (sgpos + sgsize) / (total / 100));
		fflush(stdout);
	}
...
int netflashmain(int argc, char *argv[])
{
	...
	imageVendorName[0] = NULL;
	imageProductName[0] = NULL;
	imageVersion[0] = NULL;
	...
	if (docgi) {
		...
		int parent_status;
		int count = MAX_WAIT_NETFLASH_FLUSH;
		...
		while (count > 0) {
			parent_status = kill(parent, 0);
			if ((parent_status == -1) && (errno == ESRCH)) {
				/*
