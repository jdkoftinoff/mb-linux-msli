/*
 *  linux/arch/microblaze/platform/Harman/AVB_EP/studer_legacy_bridge.c
 *
 *  Legacy packet bridge configuration driver for Studer
 *
...
static int legacy_bridge_probe(const char *name, 
  ...
  /* Announce the device */
  printk(KERN_INFO "\n%s: Found Studer Legacy Bridge at 0x%08X\n",
         bridge->name, 
...
static struct of_device_id legacy_bridge_of_match[] = {
  { .compatible = "xlnx,avb-ep-legacy-bridge-1.00.a", },
  { /* end of list */ },
};
...
{
  int returnValue;
  printk(KERN_INFO DRIVER_NAME ": Harman Studer Legacy Bridge driver\n");
...
MODULE_DESCRIPTION("Harman Studer Legacy Bridge driver");
MODULE_LICENSE("GPL");
