/*
 *  linux/arch/microblaze/platform/Harman/AVB_EP/avb_ep64_packetizer.h
 *
...
/* Driver name */
#define DRIVER_NAME "avb_ep64_packetizer"
...
static void configure_generator(struct avb_ep64_packetizer *packetizer,
...
static void reset_avb_ep64_packetizer(struct avb_ep64_packetizer *packetizer) {
  GeneratorConfig generatorConfig;
...
static int avb_ep64_packetizer_open(struct inode *inode, struct file *filp)
{
  int returnValue = 0;
  struct audio_packetizer *labxPacketizer;
  struct avb_ep64_packetizer *packetizer;

  labxPacketizer = container_of(inode->i_cdev, struct audio_packetizer, cdev);
  packetizer = (struct avb_ep64_packetizer *) labxPacketizer->derivedData;

  return(returnValue);
}

static int avb_ep64_packetizer_release(struct inode *inode, struct file *filp)
{
  struct audio_packetizer *labxPacketizer;
  struct avb_ep64_packetizer *packetizer;
...
static int avb_ep64_packetizer_ioctl(struct inode *inode, struct file *filp,
...
static struct file_operations avb_ep64_packetizer_fops = {
  .open	   = avb_ep64_packetizer_open,
  .release = avb_ep64_packetizer_release,
  .ioctl   = avb_ep64_packetizer_ioctl,
  .owner   = THIS_MODULE,
};
 ...
 */
int avb_ep64_packetizer_probe(const char *name, 
  ...
  packetizer = (struct avb_ep64_packetizer*) kmalloc(sizeof(struct avb_ep64_packetizer), GFP_KERNEL);
  ...
  returnValue = 
    audio_packetizer_probe(name, pdev, addressRange, irq,
                           &avb_ep64_packetizer_fops, packetizer,
                           &packetizer->labxPacketizer);
  if(returnValue != 0) goto free;

  /* Announce the device */
  printk(KERN_INFO "%s: Found AVB EP64 packetizer at 0x%08X\n", name,
...
#ifdef CONFIG_OF
static int avb_ep64_packetizer_platform_remove(struct platform_device *pdev);

/* Probe for registered devices */
static int __devinit avb_ep64_packetizer_of_probe(struct of_device *ofdev, const struct of_device_id *match)
{
  ...
  return(avb_ep64_packetizer_probe(name, pdev, addressRange, irq));
}

static int __devexit avb_ep64_packetizer_of_remove(struct of_device *dev)
{
  struct platform_device *pdev = to_platform_device(&dev->dev);
  avb_ep64_packetizer_platform_remove(pdev);
  return(0);
}


/* Define the devices from the tree we are compatible with */
static struct of_device_id avb_ep64_packetizer_of_match[] = {
  { .compatible = "xlnx,avb-ep-packetizer-tdm8-1.00.a", },
  { /* end of list */ },
};


static struct of_platform_driver of_avb_ep64_packetizer_driver = {
  .name		= DRIVER_NAME,
  .match_table	= avb_ep64_packetizer_of_match,
  .probe   	= avb_ep64_packetizer_of_probe,
  .remove       = __devexit_p(avb_ep64_packetizer_of_remove),
};
#endif
...
/* Probe for registered devices */
static int avb_ep64_packetizer_platform_probe(struct platform_device *pdev) {
  struct resource *addressRange;
  struct resource *irq;
  ...
  /* Dispatch to the generic function */
  return(avb_ep64_packetizer_probe(pdev->name, pdev, addressRange, irq));
}
...
static int avb_ep64_packetizer_platform_remove(struct platform_device *pdev) {
  ...
  reset_avb_ep64_packetizer(packetizer);
  kfree(packetizer);
  return(returnValue);
}

/* Platform device driver structure */
static struct platform_driver avb_ep64_packetizer_driver = {
  .probe  = avb_ep64_packetizer_platform_probe,
  .remove = avb_ep64_packetizer_platform_remove,
  .driver = {
    .name = DRIVER_NAME,
  }
};
...
static int __devinit avb_ep64_packetizer_driver_init(void)
{
  int returnValue;
  printk(KERN_INFO DRIVER_NAME ": Studer AVB EP64 Audio Packetizer driver\n");
  printk(KERN_INFO DRIVER_NAME ": Copyright(c) Harman\n");

#ifdef CONFIG_OF
  returnValue = of_register_platform_driver(&of_avb_ep64_packetizer_driver);
#endif
 
  /* Register as a platform device driver */
  if((returnValue = platform_driver_register(&avb_ep64_packetizer_driver)) < 0) {
...
static void __devexit avb_ep64_packetizer_driver_exit(void)
{
  /* Unregister as a platform device driver */
  platform_driver_unregister(&avb_ep64_packetizer_driver);
}

module_init(avb_ep64_packetizer_driver_init);
module_exit(avb_ep64_packetizer_driver_exit);

MODULE_AUTHOR("Eldridge M. Mount IV <eldridge.mount@labxtechnologies.com>");
MODULE_DESCRIPTION("Studer AVB EP64 Audio Packetizer driver");
MODULE_LICENSE("GPL");
