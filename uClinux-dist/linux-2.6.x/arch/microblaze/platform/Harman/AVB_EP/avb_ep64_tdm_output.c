/*
 *  linux/arch/microblaze/platform/Harman/AVB_EP/avb_ep64_tdm_output.c
 *
...
/* Driver name */
#define DRIVER_NAME "avb_ep64_tdm_output"
 ...
 */
#define AVB_EP64_TDM_NUM_CHANNELS  (64)
...
static void configure_analyzer(struct avb_ep64_tdm_output *tdmOutput,
...
static void get_analyzer_results(struct avb_ep64_tdm_output *tdmOutput,
...
static void configure_auto_mute(struct avb_ep64_tdm_output *tdmOutput,
...
static void reset_avb_ep64_tdm(struct avb_ep64_tdm_output *tdmOutput) {
  AnalyzerConfig analyzerConfig;
  uint32_t bankIndex;
   ...
   */
  for(bankIndex = 0; bankIndex < STREAM_MAP_BANKS; bankIndex++) {
    for(channelIndex = 0; channelIndex < AVB_EP64_TDM_NUM_CHANNELS; channelIndex++) {
...
static int avb_ep64_tdm_open(struct inode *inode, struct file *filp)
{
  int returnValue = 0;
  struct labx_local_audio_pdev *labxLocalAudio;
  struct avb_ep64_tdm_output *tdmOutput;

  labxLocalAudio = (struct labx_local_audio_pdev *) filp->private_data;
  tdmOutput = (struct avb_ep64_tdm_output *) labxLocalAudio->derivedData;

  /* Ensure the hardware is reset */
  reset_avb_ep64_tdm(tdmOutput);

  return(returnValue);
}

/* I/O control operations for the driver */
static int avb_ep64_tdm_ioctl(struct inode *inode, struct file *filp,
...
static struct file_operations avb_ep64_tdm_fops = {
  .open	   = avb_ep64_tdm_open,
  .ioctl   = avb_ep64_tdm_ioctl,
  .owner   = THIS_MODULE,
};

/* Interrupt service routine for the driver */
static irqreturn_t avb_ep64_tdm_interrupt(int irq, void *dev_id) {
  struct avb_ep64_tdm_output *tdmOutput = (struct avb_ep64_tdm_output*) dev_id;
 ...
 */
int avb_ep64_tdm_probe(const char *name, 
  ...
  tdmOutput = (struct avb_ep64_tdm_output*) kmalloc(sizeof(struct avb_ep64_tdm_output), GFP_KERNEL);
  ...
  returnValue = labx_local_audio_probe(name, 
                                       pdev, 
                                       addressRange,
                                       LA_DMA_INTERFACE_EXTERNAL,
                                       AVB_EP64_TDM_NUM_CHANNELS,
                                       &avb_ep64_tdm_fops, 
                                       tdmOutput,
  ...
  /* Announce the device */
  printk(KERN_INFO "%s: Found AVB EP64 TDM Output at 0x%08X\n", name,
  ...
  returnValue = request_irq(tdmOutput->irq, &avb_ep64_tdm_interrupt, 
                            IRQF_DISABLED, name, tdmOutput);
  if(returnValue) {
    printk(KERN_ERR "%s : Could not allocate AVB EP64 TDM Output interrupt (%d).\n",
           name, tdmOutput->irq);
    goto remove;
  }
...
#ifdef CONFIG_OF
static int avb_ep64_tdm_platform_remove(struct platform_device *pdev);

/* Probe for registered devices */
static int __devinit avb_ep64_tdm_of_probe(struct of_device *ofdev, const struct of_device_id *match)
{
  ...
  return(avb_ep64_tdm_probe(name, pdev, addressRange, irq));
}

static int __devexit avb_ep64_tdm_of_remove(struct of_device *dev)
{
  struct platform_device *pdev = to_platform_device(&dev->dev);
  avb_ep64_tdm_platform_remove(pdev);
  return(0);
}

/* Define the devices from the tree we are compatible with */
static struct of_device_id avb_ep64_tdm_of_match[] = {
  { .compatible = "xlnx,avb-ep-tdm8-output-1.00.a", },
  { .compatible = "xlnx,avb-ep-tdm8-output-1.01.a", },
  { /* end of list */ },
};


static struct of_platform_driver of_avb_ep64_tdm_driver = {
  .name        = DRIVER_NAME,
  .match_table = avb_ep64_tdm_of_match,
  .probe       = avb_ep64_tdm_of_probe,
  .remove      = __devexit_p(avb_ep64_tdm_of_remove),
};
#endif
...
/* Probe for registered devices */
static int avb_ep64_tdm_platform_probe(struct platform_device *pdev) {
  struct resource *addressRange;
  struct resource *irq;
  ...
  /* Dispatch to the generic function */
  return(avb_ep64_tdm_probe(pdev->name, pdev, addressRange, irq));
}
...
static struct platform_driver avb_ep64_tdm_driver = {
  .probe  = avb_ep64_tdm_platform_probe,
  .remove = avb_ep64_tdm_platform_remove,
  .driver = {
    .name = DRIVER_NAME,
  }
};
...
static int __devinit avb_ep64_tdm_driver_init(void)
{
  int returnValue;
  printk(KERN_INFO DRIVER_NAME ": AVB EP64 Audio TDM Output driver\n");
  printk(KERN_INFO DRIVER_NAME ": Copyright(c) Harman\n");

#ifdef CONFIG_OF
  returnValue = of_register_platform_driver(&of_avb_ep64_tdm_driver);
#endif
 
  /* Register as a platform device driver */
  if((returnValue = platform_driver_register(&avb_ep64_tdm_driver)) < 0) {
...
static void __devexit avb_ep64_tdm_driver_exit(void)
{
  /* Unregister as a platform device driver */
  platform_driver_unregister(&avb_ep64_tdm_driver);
}

module_init(avb_ep64_tdm_driver_init);
module_exit(avb_ep64_tdm_driver_exit);

MODULE_AUTHOR("Eldridge M. Mount IV <eldridge.mount@labxtechnologies.com>");
MODULE_DESCRIPTION("AVB EP64 TDM Output driver");
MODULE_LICENSE("GPL");
