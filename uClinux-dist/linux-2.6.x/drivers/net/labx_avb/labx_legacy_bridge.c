/*
 *  linux/drivers/net/labx_avb/labx_legacy_bridge.c
 *
...
static struct legacy_bridge* devices[MAX_INSTANCES] = {};
 ...
 * of data from the host to the AVB network, but permitting no
...
static void reset_legacy_bridge(struct legacy_bridge *bridge) {
  ...
  /* Hit the host MAC's Tx and Rx reset registers */
...
static int legacy_bridge_open(struct inode *inode, struct file *filp) {
  ...
  bridge = (struct legacy_bridge*)filp->private_data;
...
static int legacy_bridge_probe(const char *name, 
  ...
  /* Announce the device */
  printk(KERN_INFO "\n%s: Found Lab X Packet Legacy Bridge at 0x%08X\n",
         bridge->name, 
  ...
  bridge->pdev = pdev;
  dev_set_drvdata(bridge->miscdev.this_device, bridge);
...
static struct of_device_id legacy_bridge_of_match[] = {
  { .compatible = "xlnx,labx-legacy-bridge-1.00.a", },
  { /* end of list */ },
};
...
{
  int returnValue;
  printk(KERN_INFO DRIVER_NAME ": Lab X Legacy Bridge driver\n");
...
MODULE_DESCRIPTION("Lab X Legacy Bridge driver");
MODULE_LICENSE("GPL");
