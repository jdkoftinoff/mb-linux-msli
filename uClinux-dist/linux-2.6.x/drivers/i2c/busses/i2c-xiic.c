 ...
 * kernel.
 *
 * Adapted for MicroBlaze uClinux and backported to 2.6.30
 * by Alex Belits <alexb@meyersound.com>
 *
 */
...
struct xiic_i2c {
	...
	unsigned int 		tx_pos;
	unsigned int		nmsgs;
	unsigned long		timestamp_expire;
	unsigned char		interrupt_counter_expire;
...
{
       iowrite32/*8*/((unsigned int)value, i2c->base + reg);
}

static inline u8 xiic_getreg8(struct xiic_i2c *i2c, int reg)
{
        return (u8)ioread32/*8*/(i2c->base + reg);
}
...
static void xiic_set_timeout(struct xiic_i2c *i2c)
{
	i2c->interrupt_counter_expire=0;
...
static void __xiic_start_xfer(struct xiic_i2c *i2c)
{
	...
	while ((fifo_space >= 2) && (first || (i2c->nmsgs > 1))) {
		...
		if (i2c->tx_msg->flags & I2C_M_RD) {
			/* we dont dare putting several reads in the FIFO */
			xiic_start_recv(i2c);
			return;
		} else {
			xiic_start_send(i2c);
...
static void xiic_start_xfer(struct xiic_i2c *i2c)
{
	...
	/* initialize and disable interrupts globally */
	xiic_reinit(i2c,0);
	//	xiic_setreg32(i2c, XIIC_DGIER_OFFSET, 0);
	__xiic_start_xfer(i2c);
...
static struct of_device_id __devinitdata xiic_i2c_of_match[] = {
	{ .compatible = "xlnx,xps-iic-2.00.a", },
	{},
};
MODULE_DEVICE_TABLE(of, xilinx_i2c_of_match);
...
static struct platform_driver xiic_i2c_driver = {
	...
	.driver  = {
		.owner = THIS_MODULE,
		.name = DRIVER_NAME,
		.of_match_table = xiic_i2c_of_match,
	},
};
...
static int __devinit xiic_i2c_of_probe(struct of_device *ofdev, const struct of_device_id *match)
{
	struct xiic_i2c *i2c;
	struct xiic_i2c_platform_data *pdata;
	int ret;
	...
	pdata = (struct xiic_i2c_platform_data *) ofdev->dev.platform_data;
	/*
	if (!pdata)
		return -EINVAL;
	*/
	...
	if (!request_mem_region(r_mem->start, resource_size(r_mem), ofdev->node->name)) {
		dev_err(&ofdev->dev, "Memory region busy\n");
		ret = -EBUSY;
		goto request_mem_failed;
	}

	i2c->base = ioremap(r_mem->start, resource_size(r_mem));
	if (!i2c->base) {
	...
	ret = request_irq(r_irq->start, xiic_isr, 0, ofdev->node->name, i2c);
	if (ret) {
...
request_mem_failed:
	...
	return ret;
resource_missing:
	dev_err(&ofdev->dev, "IRQ or Memory resource is missing\n");
	return -ENOENT;
}

static int __devexit xiic_i2c_of_remove(struct of_device *ofdev)
{
	struct xiic_i2c *i2c = dev_get_drvdata(&ofdev->dev);
